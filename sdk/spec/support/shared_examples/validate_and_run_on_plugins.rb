shared_examples 'validate and run on plugins' do |
  ...
  event_keyword_name: :properties, # The name of the keyword holding combined events in the plugin method
...
|
  ...
  if context_properties
    ...
    let(:expected_merged_event) { Itly::Event.new name: expected_validation_name.to_s, properties: context_properties.merge(expected_event_properties) }
  else
  ...
  let(:response2) { Itly::ValidationResponse.new valid: !generate_validation_error, plugin_id: 'ABC', message: 'Response2 message' }
  ...
  before do
    if expect_validation
      if context_properties
        expect(itly.options.logger).to receive(:info).once.with("validate(event: <name: context, properties: #{context_properties}>)")
      end
      expect(itly.options.logger).to receive(:info).once.with("validate(event: <name: #{expected_validation_name}, properties: #{expected_event_properties}>)")
    end
    if expected_log_info
      expect(itly.options.logger).to receive(:info).once.with(expected_log_info)
    end
    expect(itly.options.logger).not_to receive(:info)

    if generate_validation_error && expect_validation
      expect(itly.options.logger).to receive(:error).with("Validation error for #{expected_validation_name}: Response2 message")
    end
    ...
    if expect_validation
      ...
      if context_properties
        expect(plugin_b).to receive(:validate).once.with(event: expected_context_event)
      end
    ...
    if expect_to_call_action
      expect(plugin_b).to receive(method).once.with(method_params.merge event_keyword_name => expected_merged_event)
    end
    expect(plugin_b).not_to receive(method)

    expect(plugin_a).to receive(:"post_#{method}").once.with(method_params.merge event_keyword_name => expected_merged_event, validation_results: all_responses)
    expect(plugin_a).not_to receive(:"post_#{method}")
    expect(plugin_b).to receive(:"post_#{method}").once.with(method_params.merge event_keyword_name => expected_merged_event, validation_results: all_responses)
